		...
		foreach( $data_content as $data_row )
		{
			if( $data_row['num_pro'] > 0 )
			{
				...
				foreach( $data_row['data'] as $data_row_i )
				{
					...
					$xtpl->assign( 'QUICK_VIEW', NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=quick_view&id=" . $data_row_i['id'] );
					$xtpl->assign( 'TITLE', $data_row_i['title'] );
					$xtpl->assign( 'TITLE0', nv_clean60( $data_row_i['title'], 40 ) );
					$xtpl->assign( 'INTRO', nv_clean60( $data_row_i['hometext'], 60 ) );
					...
					$item_last = ($i % 4 == 0) ? "item_last" : "";
			        $xtpl->assign( "item_last", $item_last );
...
{
	global $module_info, $module_name, $lang_module, $module_file, $pro_config;
					...
					if( $pro_config['active_price'] == '1' )
					{
						if( $data_row_i['showprice'] == '1' )
						{
							...
							if( $data_row_i['product_discounts'] != 0 )
							{
								...
								$xtpl->assign( 'product_discounts', CurrencyConversion( $price_product_discounts, $data_row_i['money_unit'], $pro_config['money_unit'] ) );
								$xtpl->assign( 'class_money', 'discounts_money' );
								$xtpl -> assign('sale', $data_row_i['product_discounts'] . "%");
							    $xtpl -> parse('main.catalogs.items.sale');
								$xtpl->parse( 'main.catalogs.items.price.discounts' );
								$xtpl->parse( 'main.catalogs.items.tooltip.price2.discounts2' );
							}
							else
							{
								$xtpl->assign( 'class_money', 'money' );
							...
							$xtpl->parse( 'main.catalogs.items.price' );
							$xtpl->parse( 'main.catalogs.items.tooltip.price2' );
						}
						else
						{
							$xtpl->parse( 'main.catalogs.items.contact' );
							$xtpl->parse( 'main.catalogs.items.tooltip.contact2' );
						}
					}
 ...
 * @return
 */
function quick_view( $data_content, $data_unit, $page_title )
{
	global $module_info, $lang_module, $module_file, $global_array_cat, $global_config, $module_name, $pro_config;
	
	$xtpl = new XTemplate( "quick_view.tpl", NV_ROOTDIR . "/themes/" . $module_info['template'] . "/modules/" . $module_file );
	$css = nv_html_css();
	$js = nv_html_site_js();
	$xtpl->assign( 'LANG', $lang_module );
	$xtpl->assign( 'TEMPLATE', $module_info['template'] );
	...
	if( ! empty( $data_content ) )
	{
		...
		$xtpl->assign( 'height', $pro_config['homeheight'] );
		$xtpl->assign( 'width', $pro_config['homewidth'] );
		$link_url = NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=" . $global_array_cat[$data_content['listcatid']]['alias'] . "/" . $data_content[NV_LANG_DATA . '_alias'] . "-" . $data_content['id'];
...
{
	global $module_info, $lang_module, $module_name, $module_file, $pro_config;
	...
	if( ! empty( $data_content ) )
	{
		...
		foreach( $data_content as $data_row )
		{
			...
			$xtpl->assign( 'QUICK_VIEW', NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=quick_view&id=" . $data_row['id'] );
			$pwidth = ( int )( 100 / $num_view );
			if( $i % $pro_config['per_row'] == 0 )
			{
			...
			if( $pro_config['active_price'] == '1' )
			{
				if( $data_row['showprice'] == '1' )
				{
					...
					if( $data_row['product_discounts'] != 0 )
					{
						...
						$xtpl->assign( 'product_discounts', CurrencyConversion( $price_product_discounts, $data_row['money_unit'], $pro_config['money_unit'] ) );
						$xtpl -> assign('sale', $data_row['product_discounts'] . "%");
						$xtpl -> parse('main.grid_rows.sale');
						$xtpl->assign( 'class_money', 'discounts_money' );
		...
		if( ! empty( $otherimage ) )
		{
			foreach( $otherimage as $otherimage_i )
			{
				if( ! empty( $otherimage_i ) and file_exists( NV_UPLOADS_REAL_DIR . "/" . $module_name . "/" . $otherimage_i ) )
				{
					...
					$xtpl->parse( 'main.img.othersimgs' );
				}
			}
			$xtpl->parse( 'main.img' );
		}
