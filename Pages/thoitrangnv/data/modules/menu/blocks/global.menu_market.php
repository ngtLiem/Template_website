...
if( ! nv_function_exists( 'nv_menu_site_nukeviet' ) )
{
	...
	if( ! nv_function_exists( 'nv_bmenu_active_menu' ) )
	{
		function nv_bmenu_active_menu( $cat )
		{
			...
			{
				$class = ' class="' . $cat['html_class'] . ' active"';
			}
			else
			{
				$class = '';
	...
	function nv_menu_site_nukeviet( $block_config )
	{
		...
		if( file_exists( NV_ROOTDIR . "/themes/" . $module_info['template'] . "/modules/menu/block_menu_market.tpl" ) )
		{
		...
		$xtpl = new XTemplate( "block_menu_market.tpl", NV_ROOTDIR . "/themes/" . $block_theme . "/modules/menu" );
