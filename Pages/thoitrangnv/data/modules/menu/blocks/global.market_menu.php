...
if( ! nv_function_exists( 'nv_market_menu' ) )
{
	/**
	...
	function nv_submenu_html_item( $module_array_cat, $parentid = 0 )
	{
		...
		if( ! empty( $array_item ) )
		{
			$html = "<ul class=\"ulitem\">\n";
			foreach( $array_item as $cat )
			{
				$html .= "<li class=\"lofitem1 menunongroup lofsub itemfirst\" style=\"z-index: 198;\">\n";
				$html .= "<a title=\"" . $cat['title'] . "\" href=\"" . $cat['link'] . "\"><span class=\"lof-menu-title\">" . $cat['title'] . "</span></a>\n";
				$html .= "<div class=\"divul_container level2 menunongroup\" style=\"width: 200px; display: none; top: 0px; visibility: visible;\">\n";
                $html .= "<div class=\"lof_container\">\n";
                $html .= "<div class=\"lofcolumn col1\" style=\"width:200px\">\n";
	 ...
	 * @return
	 */
	function nv_market_menu( $block_config )
	{
		global $db, $db_config, $global_config, $site_mods, $module_info, $module_name, $module_file, $module_data, $lang_global, $catid;

		if( file_exists( NV_ROOTDIR . "/themes/" . $global_config['module_theme'] . "/modules/menu/market_menu.tpl" ) )
		{
			$block_theme = $global_config['module_theme'];
		}
		elseif( file_exists( NV_ROOTDIR . "/themes/" . $global_config['site_theme'] . "/modules/menu/market_menu.tpl" ) )
		{
		...
		$xtpl = new XTemplate( "market_menu.tpl", NV_ROOTDIR . "/themes/" . $block_theme . "/modules/menu" );
		...
		foreach( $site_mods as $modname => $modvalues )
		{
			if( ! empty( $modvalues['in_menu'] ) )
			{
				...
				else
				{
					...
					{
						$array_menu['submenu'] = "<ul class=\"ulitem\">\n";
						foreach( $sub_nav_item as $item )
						{
							$array_menu['submenu'] .= "<li class=\"lofitem2 menunongroup lofnonsub itemfirst\"><a title=\"" . $item['title'] . "\" href=\"" . $item['link'] . "\">" . $item['title'] . "</a></li>\n";
						}
...
{
	$content = nv_market_menu( $block_config );
}
