<?php
$home = strtolower($page_title) == 'home' ? 'active' : '' ;
$order = strtolower($page_title) == 'my order' ? 'active' : '' ;
$info = '';
function isMobileDevice() {
  return preg_match("/(android|avantgo|blackberry|bolt|boost|cricket|docomo|fone|hiptop|mini|mobi|palm|phone|pie|tablet|up\.browser|up\.link|webos|wos)/i", $_SERVER["HTTP_USER_AGENT"]);
}
$page = strtolower($page_title) == 'home' ? 'calle luna' : $page_title ;
...
<nav class="navbar fixed-top navbar-expand-lg navbar-light white scrolling-navbar" id="main_nav">
    ...
    <div class="container">
      ...
      <img src="<?php echo base_url('uploads/rios_logo.ico') ?>" width="35px" height="35px" alt="">
        <strong class="blue-text">Calle Luna</strong>
      </a>
      ...
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        ...
        <ul class="navbar-nav nav-flex-icons pull-right">
          <li class="nav-item">
            <a class="nav-link waves-effect" href="<?php echo base_url('order/cart_list') ?>">

            <?php
              $count_cart = $this->db->query("SELECT * from cart_list where user_id = '".$_SESSION['user_id']."' ")->num_rows();
            ?>
              <span class="badge red z-depth-1 mr-1" id="cart_count" <?php echo $count_cart <= 0 ? " style='display:none' " : '' ?> > <?php echo $count_cart ?> </span>
...
<a class="mob-cart waves-effect" href="<?php echo base_url('order/cart_list') ?>">
  ...
  <div class="sidebar-wrapper">
    ...
    <div class="sidebar-field">
      ...
      <ul class="list-group sidebar-list">
        <a href="<?php echo base_url('order') ?>" class='list-link'><li class="list-group-item list-group-item-action <?php echo $home ?>">
        <i class="fa fa-home sidebar-list-icon"></i> Home
        </li></a>
        <a href="<?php echo base_url('order/my_order') ?>" class='list-link'><li class="list-group-item list-group-item-action <?php echo $order ?>">
        <i class="fa fa-list sidebar-list-icon"></i> My Orders
        </li></a>
        <a href="javascript:void(0)" class='list-link'  id="manage_account"><li class="list-group-item list-group-item-action <?php echo $info ?>">
  ...
  $(document).ready(function(){
    $('#manage_account').click(function(e){
        ...
        $('#hide-sidebar').trigger('click')
         AjaxUniModal('<?php echo base_url('order/manage_user/').$_SESSION['user_id'].'/my_account' ?>','Manage Account')
      })
