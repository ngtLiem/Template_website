      ...
      <div class="modal-dialog modal-full-height modal-left" role="document">
        ...
        <div class="modal-content">
          ...
          <div class="col-sm-12">
              <div class="row">
              <center>
                <button class="btn btn-primary" id="mark_delivered" type="button">Delivered</button>
              </center>
              </div>
          </div>
        </div>
...
$(document).ready(function(){
    ...
    $('#mark_delivered').click(function(){
        ...
        $.ajax({
            url:'<?php echo base_url('delivery/mark_delivered') ?>',
            method:'POST',
            data:{id:id},
            error:(err)=>{
                console.log(err)
            ...
            success:resp=>{
                if(resp == 1)
                websocket.send(JSON.stringify({type:'order_update',id:id}))
                $('.modal').modal('hide')
                Dtoast('Order has been marked delivered','success')
                end_loader()
            }
        })
    })
            ...
            $.ajax({
                ...
                success:resp=>{
                    if(typeof resp != undefined){
                        ...
                        $('#order_details #mark_delivered').attr('data-id',resp.details.id)
...
window.load_orders = ($id='')=>{
    $.ajax({
        url:'<?php echo base_url('delivery/load_orders') ?>',
        method:'POST',
        data:{id:$id},
        error:err=>{
            console.log(err)
